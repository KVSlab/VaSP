...
case_path, mesh_name, save_deg, stride, start_t, end_t, cut, thresh, dvp = postprocessing_common_h5py.read_command_line_spi()
...
if os.path.exists(formatted_data_path):
    ...
    print('create ' + dvp + ' file using "create_domain_specific_viz.py" before running spi script!')
...
postprocessing_common_h5py.calculate_spi(case_name, df, visualization_separate_domain_folder, mesh_path, start_t, end_t,"low",25, dvp)
postprocessing_common_h5py.calculate_spi(case_name, df, visualization_separate_domain_folder, mesh_path, start_t, end_t,"low",200, dvp)
postprocessing_common_h5py.calculate_spi(case_name, df, visualization_separate_domain_folder, mesh_path, start_t, end_t,"low",400, dvp)
postprocessing_common_h5py.calculate_spi(case_name, df, visualization_separate_domain_folder, mesh_path, start_t, end_t,"high",25, dvp)
