...
def compute_flow_rate(case_path,mesh_name, dt, stride, save_deg):
    ...
    for file in os.listdir(case_path):
        ...
        if os.path.exists(os.path.join(file_path, "1")):
            visualization_path = os.path.join(file_path, "1/Visualization_sd1")
        elif os.path.exists(os.path.join(file_path, "Visualization_sd1")):
            visualization_path = os.path.join(file_path, "Visualization_sd1")
    
    visualization_path = Path(visualization_path)
    file_path_v = visualization_path / "velocity_save_deg_1.h5"
    file_path_flow_rate = visualization_path / "flow_rates.txt"
    xdmf_file = visualization_path / "velocity_save_deg_1.xdmf"
