...
#        dx_s_id=2,      # ID of marker in the solid domain
        ...
        folder="Resistance_BC",# 
        checkpoint_step=5, # 
        save_deg=1 # 1 saves the nodal values only while 2 takes full advantage of the mide side nodes available in the P2 solution. P2 f0r nice visualisations :)
        # probe point(s)
    ))
...
def get_mesh_domain_and_boundaries(mesh_file, fsi_id, rigid_id, outer_wall_id, folder, **namespace):
    ...
    # Only considere FSI in domain within this sphere # Corrected sphere for the rescaled artery!
    sph_x = 1.8124701455235481e-06
    sph_y = -6.299931555986404e-06
    sph_z = -7.300404831767082e-07
    sph_rad = 0.0034
    print("The coordinates of the FSI sphere are [{}, {}, {}] of radius:{}".format(sph_x, sph_y, sph_z, sph_rad))
...
class VelInPara(UserExpression):
    ...
    def eval(self, value, x):
        if self.t < 0.1:
            v_local_max = self.t*(0.74/0.1) # 0.77 m/s mmHg rise in 0.1s
        else:
            v_local_max = self.t*0.9666667 + 0.64333333 # lower slope

        r2 = (x[0]-self.c[0])**2 + (x[1]-self.c[1])**2 + (x[2]-self.c[2])**2  # radius**2
        fact_r = 1 - (r2/self.r**2)

        value[0] = -self.n[0] * (v_local_max) *fact_r  # *self.t
        value[1] = -self.n[1] * (v_local_max) *fact_r  # *self.t
        value[2] = -self.n[2] * (v_local_max) *fact_r  # *self.t
        #print("X: {}, Y: {}, Z: {}".format(value[0],value[1],value[2]))
...
class InnerP(UserExpression):
    ...
    def update(self, t):
        ...
        diam = 0.005 # metres
        V_init = 0.77 # m/s
        Area_Inlet = (3.14159/4)*diam**2
        p_final_ramp = self.resistance*(V_init/2)*Area_Inlet    
        ...
        self.P = ramp_factor * (self.p_0 + self.resistance * Q)
        if MPI.rank(MPI.comm_world) == 0:
...
def create_bcs(dvp_, DVP, mesh, boundaries, domains, mu_f,
               ...
               rigid_id, psi, F_solid_linear,p_deg, **namespace):
    ...
    p_out_bc_val = InnerP(t=0.0, t_ramp=0.02, n=n, u=sol[0], dsi=dsi, resistance=1e10, p_0=0.0, degree=p_deg)
...
def pre_solve(t, v_, u_inflow_exp, p_out_bc_val, **namespace):
